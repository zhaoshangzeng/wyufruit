    ...
    <div class="other_tit">我的订单</div>
      <div class="other_main"  style="background-image: url('${pageContext.request.contextPath }/img/gwcbj.jpg'); background-size: cover;">
        ...
        <c:if test="${listorder != null }">
          ...
          <h4>结算 清单</h4>
          <ul>
          	<c:forEach var="one" items="${listorder }">
            	<li>${one.name }<i>-</i> <span>￥${one.totalPrice }</span></li>
          	</c:forEach>
            <li>合计 <i>：</i> <span>￥${orderSum }</span></li>
            <c:if test="${hyPrice != null}">
            	<li>会员最终价 <i>：</i> <span>￥${hyPrice }</span></li>
            </c:if>
          </ul>
        </div>
        </center>
		 ...
		 <div class="checkout-left-basket animated wow slideInRight" data-wow-delay=".5s">
          <a href="${pageContext.request.contextPath }/Buy"><span class="glyphicon glyphicon-menu-left" aria-hidden="true"></span>确认 购买</a>
        </div>
